function [samples, logprobs, accept_rate] = Mcmc(obs,guess_p_start,guess_p_trans,guess_mu_emit,guess_sigma_emit,mcmc_params)
%%%%%%%%%%%%%%%%%%%%
...
if iscell(obs)
    for i=1:length(obs)
        logprobs(1) = logprobs(1) + HMM_Bayes.Forward(obs{i},samples(1).p_start,samples(1).p_trans,samples(1).mu_emit,samples(1).sigma_emit);
    end
else
    logprobs(1) = HMM_Bayes.Forward(obs,samples(1).p_start,samples(1).p_trans,samples(1).mu_emit,samples(1).sigma_emit);
end
...
for i=2:mcmc_params.nIter
    ...
    if ~reject
        ...
        if iscell(obs)
            for j=1:length(obs)
                logprobs(i) = logprobs(i) + HMM_Bayes.Forward(obs{j},samples(i).p_start,samples(i).p_trans,samples(i).mu_emit,samples(i).sigma_emit);
            end
        else
            logprobs(i) = HMM_Bayes.Forward(obs,samples(i).p_start,samples(i).p_trans,samples(i).mu_emit,samples(i).sigma_emit);
        end
