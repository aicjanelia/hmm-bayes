function logI = IntegrationGaussianChi(obs,d,samples_mcmc,N,Vstates)
%%%%%%%%%%%%%%%%%%%%
...
for i=1:N
    ...
    if iscell(obs)
        logprob = 0;
        for j=1:length(obs)
            logprob = logprob + HMM_Bayes.ForwardChi(obs{j},d,p_start,p_trans,mu_emit,sigma_emit);
        end
    else
        logprob = HMM_Bayes.ForwardChi(obs,d,p_start,p_trans,mu_emit,sigma_emit);
    end
